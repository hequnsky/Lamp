...
import com.rmc.lamp.utils.Util;
...
public class RoadAdapter extends RecyclerView.Adapter<RoadAdapter.RoadViewHolder> {
    ...
    public void onBindViewHolder(RoadViewHolder holder, int position) {
        ...
        if (position == 0) {
            ...
            marginLayoutParams.width = Util.dp2px(mContext, (float) (150));
        } else {
            int random = (int) (Math.random() * 100);
            if (random > 50) {
                holder.itemView.setTag("W");
                holder.itemView.setBackgroundColor(Color.WHITE);
                marginLayoutParams.width = Util.dp2px(mContext, (float) (30 + Math.random() * 50));
            } else {
                holder.itemView.setTag("B");
                holder.itemView.setBackgroundColor(Color.BLACK);
                marginLayoutParams.width = Util.dp2px(mContext, (float) (50 + Math.random() * 30));
            }
        }
